...
namespace ExitGames.Demos.DemoAnimator
{
	...
	public class PlayerUI : MonoBehaviour
    {
		...
		public void SetTarget(PlayerManager target){
            ...
            Debug.Log("setting my ui");
            
			// Cache references for efficiency because we are going to reuse them.
			_target = target;
			_targetTransform = _target.GetComponent<Transform>();
			_targetRenderer = _target.GetComponent<Renderer>();
            // PlayerPrefs.GetFloat("gr") * 1000 + PlayerPrefs.GetInt("lvl") * 10
            /*
            if (target.photonView.isMine)
            {
                this.gameObject.GetComponent<Slider>().maxValue = PlayerPrefs.GetFloat("gr") * 1000 + PlayerPrefs.GetInt("lvl") * 10;
            }
            */
            this.gameObject.GetComponent<Slider>().maxValue = _target.gameObject.GetComponent<PlayerManager>().getHealth();
        ...
        public void OnPhotonSerializeView(PhotonStream stream, PhotonMessageInfo info)
        {
            if (stream.isWriting)
            {
                ...
                stream.SendNext(this.gameObject.GetComponent<Slider>().maxValue);
            }
            else
            {
                // Network player, receive data
                
                this.gameObject.GetComponent<Slider>().maxValue = (float)stream.ReceiveNext();
            }
        }
        */
